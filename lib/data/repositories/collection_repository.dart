    ...
    if (target != null) {
      await (_db.update(_db.collection)..whereSamePrimaryKey(target)).write(
        CollectionCompanion(
          ...
          priceEur: Value(card.prices.eur),
          priceEurFoil: Value(card.prices.eurFoil),
  ...
  Future<int> restore(CollectionData row) =>
      _db.into(_db.collection).insert(
            CollectionCompanion.insert(
              ...
              priceEur: Value(row.priceEur),
              priceEurFoil: Value(row.priceEurFoil),
